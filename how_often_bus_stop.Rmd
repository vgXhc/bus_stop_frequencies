...
We can clearly see the four transfer points; a cluster of stations with frequent service along the University Ave/Johnson St couplet, State St, and on the Capitol Square; and one busy quasi-transfer point at East Towne Mall.
...
This map looks quite different: The transfer points lose a lot of prominence, whereas the stops on the UW Campus really stand out. Let's look at the top 15 stops in a table.
...
The Observatory & Elm EB stop on average has on average 21 passengers boarding each bus. The highest non-UW stop: The stop serving the Epic campus in Verona, with only 30 daily departures. There are only two other non-UW stops in the list: One serves the Madison College campus on the north side, and one is a Park & Ride lot.
...
The reason for the high raw number of boardings is merely that the current route system is set up in a way that forces riders from outlying areas to transfer at the transfer points to get downtown or across town.
